...
dev-assets:  ## run update_assets to copy required statics in local volumes
	$(COMPOSE_RUN) --no-deps lms-dev paver update_assets lms --settings devstack_decentralized --skip-collect
.PHONY: dev-assets
...
fetch-demo: \
  check-activate \
  check-root-user \
  $(FLAVORED_EDX_RELEASE_PATH)/edx-demo-course/README.md
fetch-demo:  ## fetch openedx demo course
	@echo "Demo course release '$(EDX_DEMO_RELEASE_REF)' is available at: $(FLAVORED_EDX_RELEASE_PATH)/edx-demo-course/"
.PHONY: fetch-demo
...
fetch-release:  ## fetch openedx release sources
	@echo "Release '$(EDX_RELEASE_REF)' is available at: src/edx-platform/"
.PHONY: fetch-release
